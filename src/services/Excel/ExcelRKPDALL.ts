...
export const exportRKPDALL = async (
  ...
  for (const skpd of listSKPDPeriode) {
    ...
    const fixedCells: Array<{ addr: string; value: string; style?: Partial<ExcelJS.Style> }> = [
      ...
      { addr: 'F9', value: `Target RPJMD Kabupaten/kota pada Tahun ${tahunLabel}\n(Akhir Periode RPJMD)` },
      ...
      { addr: 'V9', value: `Realisasi Kinerja dan Anggaran RPJMD Kabupaten/kota s/d Tahun ${tahunLabel})` },
      { addr: 'V11', value: '14 = 7 + 13' }, { addr: 'V12', value: 'K' }, { addr: 'W12', value: 'Rp' },

      { addr: 'X9', value: `Tingkat Capaian Kinerja dan Realisasi Anggaran RPJMD Kabupaten/kota s/d Tahun ${tahunLabel}\n(%)` },
    ...
    data.forEach((item, idx) => {
      ...
      // 16
      row.getCell('Z').value = skpd.label;
  ...
  saveAs(blob, `[SELURUH SKPD] Laporan Evaluasi Terhadap RKPD Kabupaten Bengkulu Utara Tahun ${tahunLabel} ${waktuNowGabung}.xlsx`);
};