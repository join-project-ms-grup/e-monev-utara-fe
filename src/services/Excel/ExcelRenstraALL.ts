...
import { getCatatanEvaluasi } from '../CatatanService';
import { flatRenstraNew, getRenstraNew } from '../RenstraService';
import type { OptionItem } from '../../components/inputs/InputSearchBox';

export const exportRenstraALL = async (
  ...
  for (const skpd of listSKPDPeriode) {
    ...
    const catatan = await getCatatanEvaluasi({
      skpd_periode_id: id,
      type: 'renstra',
    });

    const rawData = await getRenstraNew(id);
    const data = flatRenstraNew(rawData as any);
    ...
    [
      ...
      { col: 'A3', value: `Renstra Perangkat Daerah ${skpd.label} Kabupaten Bengkulu Utara` },
    ...
    data.forEach((item, idx) => {
      ...
      // 22
      row.getCell('AL').value = skpd.label;
  ...
  saveAs(blob, `[SELURUH SKPD] Evaluasi Terhadap Hasil Renstra Perangkat Daerah Lingkup Kabupaten/kota ${getPeriodeMulaiFromCookie()} - ${getPeriodeAkhirFromCookie()} ${waktuNowGabung}.xlsx`);
};