...
import { getCatatanEvaluasiRKPD } from '../CatatanService';
import { flatRKPDTriwulan, getRKPDTriwulan } from '../RKPDService';

export const exportRenjaALL = async (
    listSKPDPeriode: OptionItem[],
    tahunLabel: string,
    tahunId: number,
    ...
    for (const skpd of listSKPDPeriode) {
        const worksheet = workbook.addWorksheet(skpd.label);
        const id = Number(skpd.value);

        const catatan = await getCatatanEvaluasiRKPD({
            skpd_periode_id: id,
            type: 'renja',
        });
        ...
        const data = flatRKPDTriwulan(rawData as any);
        ...
        const fixedCells: Array<{ addr: string; value: string; style?: Partial<ExcelJS.Style> }> = [
            ...
            { addr: 'A3', value: `Renja Perangkat Daerah ${skpd.label} Kabupaten Bengkulu Utara` },
            { addr: 'A4', value: `Tahun ${tahunLabel}` },
        ...
        data.forEach((item, idx) => {
            ...
            // 15
            row.getCell('Y').value = skpd.label;
        ...
        const rowsConfig = [
            ...
            { offset: 3, merge: 'A:Y', text: `Faktor pendorong keberhasilan kinerja: ${catatan.pendorong}` },
            { offset: 4, merge: 'A:Y', text: `Faktor penghambat pencapaian kinerja: ${catatan.penghambat}` },
            { offset: 5, merge: 'A:Y', text: `Tindak lanjut yang diperlukan dalam triwulan berikutnya*): ${catatan.tl_1}` },
            { offset: 6, merge: 'A:Y', text: `Tindak lanjut yang diperlukan dalam Renja Perangkat Daerah kabupaten/kota berikutnya*): ${catatan.tl_2}` }
        ];
    ...
    saveAs(blob, `[SELURUH SKPD] Evaluasi Terhadap Hasil Renja Lingkup Kabupaten Bengkulu Utara Periode Pelaksanaan: tahun ${awalPeriode} - tahun ${akhirPeriode} ${waktuNowGabung}.xlsx`);
};