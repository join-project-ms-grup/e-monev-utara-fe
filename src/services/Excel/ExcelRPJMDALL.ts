...
export const exportRPJMDALL = async (
    ...
    for (const skpd of listSKPDPeriode) {
        ...
        const raw = await getRPJMDNew(id);
        const data = flatRPJMDNew(raw as any);
    ...
    saveAs(blob, `[SELURUH SKPD] Evaluasi Terhadap Hasil RPJMD Kabupaten Bengkulu Utara Periode Pelaksanaan: tahun ${awalPeriode} - tahun ${akhirPeriode} ${waktuNowGabung}.xlsx`);
};